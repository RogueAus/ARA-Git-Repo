...
_spawnPoint = "";
	...
	if(EQUAL(typeName _spawnPoints,typeName [])) then {
		...
		{if(count(nearestObjects[(getMarkerPos _x),["Car","Ship","Air","Tank","Car_F","Truck_F"],5]) == 0) exitWith {_spawnPoint = _x};} foreach _spawnPoints;
	} else {
		if(count(nearestObjects[(getMarkerPos _spawnPoints),["Car","Ship","Air","Tank","Car_F","Truck_F"],5]) == 0) exitWith {_spawnPoint = _spawnPoints};
	};
