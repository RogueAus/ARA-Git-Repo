...
[] spawn {
	while {true} do 
	{
		DGTFR_server = call TFAR_fnc_getTeamSpeakServerName;
		DGTFR_chan = call TFAR_fnc_getTeamSpeakChannelName;
	
		if(!([] call TFAR_fnc_isTeamSpeakPluginEnabled)) then {
			if(FETCH_CONST(life_adminlevel) isEqualTo 0) then {
				titleText["Enable TFR Plugin on Teamspeak!","BLACK"];
				waitUntil {([] call TFAR_fnc_isTeamSpeakPluginEnabled)};
				titleText["","PLAIN"];
			};
		};
		
		if(DGTFR_server != "Division Gaming Australia" or DGTFR_chan != "TakiLifeRadio") then {
			if(FETCH_CONST(life_adminlevel) isEqualTo 0) then {
				titleText["Wrong TS Channel. Please join the correct channel.","BLACK",0,true];
				waitUntil{([] call TFAR_fnc_getTeamSpeakServerName) == "Division Gaming Australia" and ([] call TFAR_fnc_getTeamSpeakChannelName) == "TakiLifeRadio"};
				titleText["","PLAIN"]; 
			};
	    };
		sleep 5;
