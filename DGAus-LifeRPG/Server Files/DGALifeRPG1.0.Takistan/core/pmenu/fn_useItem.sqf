...
switch (true) do {
	...
	case (EQUAL(_item,"bleach")): {
		[] spawn {
			[false,"bleach",1] call life_fnc_handleInv;
			["red","You don't feel so good."] call life_fnc_dgmsg;
			player forceWalk true;
			["red","You must get to a hospital."] call life_fnc_dgmsg;
			if(damage player < 0.1) then {player setDamage 0.1};
			sleep 5;
			while{damage player > 0.09 && alive player} do {
				sleep 5;
				if(damage player > 0.09) then {player setDamage (damage player + 0.01)};
			};
			if(!alive player) exitWith {["red","You have died from poisoning."] call life_fnc_dgmsg;};
			["green","You have been healed."] call life_fnc_dgmsg;
		};
	};
