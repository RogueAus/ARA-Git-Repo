...
switch (_code) do {
	//Space key for Jumping - Obsolete bc of Advanced Movement
	/*case 57: {
	...
	case 33: {
		if(playerSide in [west,independent] && {vehicle player != player} && {!life_siren_active} && {((driver vehicle player) == player)}) then {
			_veh = vehicle player;
			if(!(typeOf _veh in ["C_Offroad_01_F","B_MRAP_01_F","C_SUV_01_F","C_Hatchback_01_sport_F","sab_UN_Offroad","sab_UN_SUV","sab_UN_Hunter","DG_TSF_ARMED_OFFROAD","DG_TSF_Offroad","DG_TSF_SUV","DG_TSF_HUNTER"])) exitWith {};
			[] spawn {
	...
	//DGAus Custom Keys
	// O key = Open Boom Gates cop/medic
    case 24:
	{
