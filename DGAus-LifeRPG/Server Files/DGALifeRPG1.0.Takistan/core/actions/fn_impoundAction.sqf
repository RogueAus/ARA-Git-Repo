...
*/
private["_vehicle","_type","_time","_price","_vehicleData","_upp","_ui","_progress","_pgText","_cP","_filters","_nearObjects"];
_vehicle = param [0,ObjNull,[ObjNull]];
_filters = ["Car","Air","Ship","Tank","Car_F","Truck_F"];
...
if(EQUAL(count crew _vehicle,0)) then {
	...
	switch (true) do {
		...
		case (_vehicle isKindOf "Tank"): {_price = LIFE_SETTINGS(getNumber,"impound_car");};
		case (_vehicle isKindOf "Car_F"): {_price = LIFE_SETTINGS(getNumber,"impound_car");};
		case (_vehicle isKindOf "Truck_F"): {_price = LIFE_SETTINGS(getNumber,"impound_car");};
	};
	
	life_impound_inuse = true;
	
	_nearObjects = nearestObjects[getPos _vehicle,["newlightbar","bumper","back_lights","antenna","strobe_lights","big_bumper","big_back_lights"],4];
	{
		deleteVehicle _x;
