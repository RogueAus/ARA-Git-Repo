...
if(((nearestObject [[5654.92,8896.03,0.4],"Land_Research_house_V1_F"]) == _building) && ({side _x == west} count playableUnits < (LIFE_SETTINGS(getNumber,"cops_online_min")))) exitWith {
	hint format [localize "STR_Civ_NotEnoughCops",(LIFE_SETTINGS(getNumber,"cops_online_min"))]
};
if(isNil "life_boltcutter_uses") then {life_boltcutter_uses = 0;};

if((nearestObject [[5654.92,8896.03,0.4],"Land_Research_house_V1_F"]) == _building) then {
...
switch (typeOf _building) do {
	case "Land_Research_house_V1_F": {if(life_fed_break == 0) then {life_fed_break = 1;};};
};
